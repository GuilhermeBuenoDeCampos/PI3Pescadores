...
export default function Register() {
  useEffect(() => {
    const form = document.getElementById('tp-register-form');
    if (!form) return;
    ...
    const confirmPassword = document.getElementById('tp-confirm-password');
    const toggle = document.getElementById('tp-toggle-password');
    const toggleConfirm = document.getElementById('tp-toggle-confirm-password');
    ...
    const nameError = document.getElementById('tp-name-error');
    ...
    const confirmPasswordError = document.getElementById('tp-confirm-password-error');
    ...
    function validateNameField(){ const v = name.value.trim(); if (!v){ showError(nameError,'O nome é obrigatório.'); return false } if (v.length < 3){ showError(nameError,'Nome deve ter ao menos 3 caracteres.'); return false } hideError(nameError); return true }
    ...
    function validateCPFField(){ const v = cpf.value.trim(); if (!v){ showError(cpfError,'O CPF é obrigatório.'); return false } if (!validateCPF(v)){ showError(cpfError,'CPF inválido.'); return false } hideError(cpfError); return true }
    function validateUsernameField(){ const v = username.value.trim(); if (!v){ showError(usernameError,'O usuário é obrigatório.'); return false } if (v.length < 2){ showError(usernameError,'Usuário muito curto.'); return false } hideError(usernameError); return true }
    ...
    function validateConfirmPasswordField(){ const v = confirmPassword.value; if (!v){ showError(confirmPasswordError,'Confirmação de senha é obrigatória.'); return false } if (v !== password.value){ showError(confirmPasswordError,'Senhas não correspondem.'); return false } hideError(confirmPasswordError); return true }

    function validateForm(){ const a = validateNameField(); const b = validateEmailField(); const c = validateCPFField(); const d = validateUsernameField(); const e = validatePasswordField(); const f = validateConfirmPasswordField(); const ok = a && b && c && d && e && f; submitBtn.disabled = !ok; return ok }

    name.addEventListener('input', ()=>{ validateNameField(); validateForm(); });
    ...
    password.addEventListener('input', ()=>{ validatePasswordField(); validateConfirmPasswordField(); validateForm(); });
    confirmPassword.addEventListener('input', ()=>{ validateConfirmPasswordField(); validateForm(); });
    ...
    toggleConfirm.addEventListener('click', ()=>{
      const isPwd = confirmPassword.type === 'password'; confirmPassword.type = isPwd ? 'text' : 'password'; toggleConfirm.setAttribute('aria-pressed', String(isPwd));
      toggleConfirm.querySelectorAll('svg').forEach(s => s.classList.toggle('hidden'));
    });
    ...
    form.addEventListener('submit', function(e){ e.preventDefault(); formStatus.textContent = ''; formStatus.className = '';
      ...
      submitBtn.disabled = true; btnText.textContent = 'Criando...'; btnSpinner.classList.remove('hidden'); 
      setTimeout(()=>{ btnSpinner.classList.add('hidden'); btnText.textContent = 'Criar Conta'; formStatus.textContent = 'Cadastro realizado com sucesso!'; formStatus.className = 'text-sm text-green-600 mt-2'; submitBtn.disabled = !validateForm(); formStatus.setAttribute('role','alert'); }, 1500);
    });

    validateForm();

    return () => {
      try { name.removeEventListener('input', ()=>{}); } catch(e){}
    }
  }, []);
  ...
  return (
    <div className="min-h-screen flex bg-white" style={{backgroundColor: '#faf8f3'}}>
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center px-4 md:px-8 py-6 overflow-y-auto" style={{backgroundColor: '#3d4e8f'}}>
        <div className="max-w-md mx-auto w-full">
          <h1 className="text-3xl md:text-4xl font-extrabold text-white mb-1 text-center">Criar Conta</h1>
          <p className="text-indigo-100 text-sm mb-4 text-center">Preencha os dados para se cadastrar na plataforma</p>

          <form id="tp-register-form" noValidate className="space-y-2">
            <div>
              <label htmlFor="tp-name" className="block text-white text-sm font-semibold mb-1">Nome Completo</label>
              <input id="tp-name" name="name" type="text" autoComplete="name"
                     ...
                     placeholder="Seu nome completo" aria-describedby="tp-name-error" required />
              <p id="tp-name-error" className="mt-1 text-sm text-red-300 hidden" role="alert" aria-live="assertive"></p>
            </div>
            ...
            <div>
              <label htmlFor="tp-cpf" className="block text-white text-sm font-semibold mb-1">CPF</label>
              <input id="tp-cpf" name="cpf" type="text" inputMode="numeric" placeholder="000.000.000-00"
                     className="w-full px-4 py-2 bg-white/20 border border-white/30 rounded-lg text-white placeholder-indigo-200 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent transition" 
                     aria-describedby="tp-cpf-error" required />
              <p id="tp-cpf-error" className="mt-1 text-sm text-red-300 hidden" role="alert" aria-live="assertive"></p>
            </div>

            <div>
              <label htmlFor="tp-username" className="block text-white text-sm font-semibold mb-1">Usuário</label>
              <input id="tp-username" name="username" type="text" autoComplete="username"
                     className="w-full px-4 py-2 bg-white/20 border border-white/30 rounded-lg text-white placeholder-indigo-200 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent transition" 
                     placeholder="seu_usuário" aria-describedby="tp-username-error" required />
            ...
            <div>
              ...
              <div className="relative">
                <input id="tp-password" name="password" type="password" autoComplete="new-password"
            ...
            <div>
              <label htmlFor="tp-confirm-password" className="block text-white text-sm font-semibold mb-1">Confirmar Senha</label>
              <div className="relative">
                <input id="tp-confirm-password" name="confirmPassword" type="password" autoComplete="new-password"
                       className="w-full px-4 py-2 pr-12 bg-white/20 border border-white/30 rounded-lg text-white placeholder-indigo-200 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-transparent transition" 
                       placeholder="••••••••" aria-describedby="tp-confirm-password-error" required />
                <button type="button" id="tp-toggle-confirm-password" aria-label="Mostrar senha" className="absolute right-3 top-1/2 -translate-y-1/2 text-indigo-200 hover:text-white transition">
                  <svg id="tp-eye-open" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                  <svg id="tp-eye-closed" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.477 0-8.268-2.943-9.542-7a9.965 9.965 0 012.642-4.206M6.1 6.1l11.8 11.8M19.9 6.1l-1.5 1.5"/></svg>
                </button>
              </div>
              <p id="tp-confirm-password-error" className="mt-1 text-sm text-red-300 hidden" role="alert" aria-live="assertive"></p>
            </div>
            ...
            <button id="tp-submit" type="submit" className="w-full py-2 px-4 mt-4 font-semibold rounded-lg text-white hover:shadow-lg transition flex items-center justify-center gap-2" style={{backgroundColor: '#2e9e82'}} disabled>
              ...
              <span id="tp-btn-text">Criar Conta</span>
            </button>
          ...
          <div className="mt-4 pt-4 border-t border-white/20 text-center text-sm">
            <p className="text-indigo-200">Já tem uma conta? <Link to="/login" className="text-white font-semibold hover:text-teal-200 transition">Entrar</Link></p>
          </div>
        </div>
      </div>
